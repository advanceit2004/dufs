    ...
    let app = Command::new(env!("CARGO_CRATE_NAME"))
        ...
        .arg(
            Arg::new("gen-config")
                .long("gen-config")
                .action(ArgAction::SetTrue)
                .help("Print an annotated starter config file (redirect to config.yaml)"),
        );
...
/// Annotated starter config printed by `dufs --gen-config`.
/// Demonstrates the business auth syntax, including the common
/// "sub-folder admin who cannot modify the root" pattern.
pub const STARTER_CONFIG: &str = r#"# dufs starter config — generated by `dufs --gen-config`
# Usage: dufs --config config.yaml

# The folder to serve
serve-path: '.'
# Listen port
port: 5000
# Enable upload, delete, search, archive (restrict per user below)
allow-all: true

auth:
  # Users listed here can open the permission-management overview in the web UI.
  # This only controls UI visibility of permission metadata, not file access.
  admins:
    - admin

  users:
    # Full administrator: read-write everywhere.
    admin:
      password: 'CHANGE-ME'
      paths:
        - '/:rw'

    # Sub-folder manager: can fully manage the listed folders, but the
    # root stays index-only (no uploads, deletes, or new folders at root).
    portaladmin:
      password: 'CHANGE-ME'
      paths:
        - '/ProjectA:rw'
  ...
  # Groups grant shared paths/roles to their members.
  groups: {}
  #  team:
  #    members: [portaladmin]
  #    paths: ['/shared:rw']

  # Roles are named permission sets assignable to users or groups.
  roles: {}
  #  editor:
  #    description: 'Can edit team files'
  #    paths: ['/team:rw']
"#;
...
mod tests {
    ...
    #[test]
    fn test_example_config_matches_starter_config() {
    ...
    fn test_starter_config_is_valid() {
        ...
        config_file.write_str(STARTER_CONFIG).unwrap();
        ...
        assert!(args.auth.is_admin(Some("admin")));
        assert!(!args.auth.is_admin(Some("portaladmin")));
    }
